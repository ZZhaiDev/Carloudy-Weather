...
//        layout.sectionInset = UIEdgeInsets(top: 0, left: kEdgeMargin, bottom: 0, right: kEdgeMargin)
        ...
        layout.headerReferenceSize = CGSize(width: zjScreenWidth, height: 200)
        let cv = UICollectionView(frame: CGRect(x: 0, y: 0, width: zjScreenWidth, height: 300), collectionViewLayout: layout)
        cv.backgroundColor = .clear
        cv.delegate = self
        cv.dataSource = self
        cv.register(MainContentViewCell.self, forCellWithReuseIdentifier: cellId)
//        cv.register(MainContentViewCellHeader.self,forsu, forCellWithReuseIdentifier: headerCellId)
...
//        cv.register(MainContentViewCellHeader.self, viewForSupplementaryElementOfKind: UICollectionView.elementKindSectionHeader, at: headerCellId)
        return cv
    }()
...
//    }()
    ...
    let tableView: MainContentTableView = {
        let tv = MainContentTableView(frame: CGRect(x: 0, y: 0, width: zjScreenWidth, height: zjScreenHeight - 300))
